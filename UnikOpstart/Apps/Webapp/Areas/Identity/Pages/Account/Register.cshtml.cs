        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (Input.LoginType == "Medarbejder")
                    {
                        //Dto for medarbejder
                        var dto = new CreateRequestDtoMedarbejder //Add user to medarbejder.
                        {
                            UserId = userId,
                            Name = Input.Name,
                        ...
                        await _medarbejderService.Create(dto); //create medarbejder
                        await _userManager.AddClaimAsync(user, new Claim("role", "Medarbejder")); //Adds claim to registered user
                    }
                    else if (Input.LoginType == "Kunde")
                    {
                        //Dto for kunde
                        var dto = new CreateRequestDtoKunde //Add user to medarbejder.
                        {
                            UserId = userId,
                            Name = Input.Name,
                            Email = Input.Email,
                        ...
                        await _kundeService.Create(dto); //create medarbejder
                        await _userManager.AddClaimAsync(user, new Claim("role", "Kunde")); //Adds claim to registered user
                    }
